...
@php
    ...
    $questionsWithTheSamePractice = [];
/*    foreach ($questions as $question){
        if($question->practice_id == $project->practice_id){
                array_push($questionsWithTheSamePractice,$question);
...
@foreach ($questions as $question)
    @switch($question->originalQuestion->type)
        ...
        <div class="form-group questionDiv {{$class}}"
            ...
            <label>{{$question->originalQuestion->label}}{{$question->originalQuestion->required ? '*' : ''}}{!!$question->shouldShow ? ' <span style="color:green">Validated</span>' : ''!!}</label>
            <input
                {{$required ? 'required' : ''}}
                {{$question->originalQuestion->required ? 'required' : ''}}
                {{$disabled ? 'disabled' : ''}}
                {{$question->shouldShow ? 'disabled' : ''}}
                class="form-control"
                type="text"
        ...
        <div class="form-group questionDiv {{$class}}"
            ...
            <select
                ...
                {{$question->originalQuestion->required ? 'required' : ''}}
            >
                <option @if($question->response == '') selected
                        @endif disabled="">{{$question->originalQuestion->placeholder}}</option>
                ...
                @else
                    @foreach ($question->originalQuestion->optionList() as $option)
                        <option value="{{$option}}"
                                @if($question->response == $option) selected @endif>{{$option}}</option>
                    @endforeach
                @endif
            </select>
        </div>
        ...
        <div class="form-group questionDiv {{$class}}"
            ...
            >
                ...
                @else
                    @foreach ($question->originalQuestion->optionList() as $option)
                        <option
                            value="{{$option}}" {{in_array($option, $selectedOptions) ? 'selected' : ''}}>{{$option}}</option>
                    @endforeach
                @endif
            </select>
        </div>
        ...
        <div class="form-group questionDiv" data-practice="{{$question->originalQuestion->practice->id ?? ''}}">
            <x-questionFileUploader
                ...
                :disabled="$disabled"
                :disabled="$question->shouldShow"
                :required="$required"
            />
        </div>
        @if (!$disabled)
            <p style="font-size: 12px">
                Do not include personal, sensitive data, personal data relating to criminal convictions and offences or
                financial data
                in this free form text field or upload screen shots containing personal data, unless you are consenting
                and assuming
                responsibility for the processing of this personal data (either your personal data or the personal data
                of others) by
                Accenture.
            </p>
            <br>
