...
class InnovationTest extends TestCase
{
    ...
    public function testAccentureProjectBenchmarksInnovationWithoutProject()
    {
        $user = factory(User::class)->states('accenture')->create();

        $response = $this
            ->actingAs($user)
            ->get('/accenture/project/benchmark/innovation');

        $response->assertStatus(404);
    }

    public function testAccentureProjectBenchmarksInnovationWithProject()
    {
        $user = factory(User::class)->states('accenture')->create();
        ...
        $response = $this
            ->actingAs($user)
            ->get('/accenture/project/benchmark/innovation/' . $project->id);
