...
class ProjectConclusionsTest extends TestCase
{
    ...
    public function testAccentureProjectConclusionsWithoutProject()
    {
        $user = factory(User::class)->states('accenture')->create();

        $response = $this
            ->actingAs($user)
            ->get('/accenture/project/conclusions');

        $response->assertStatus(404);
    }

    public function testAccentureProjectConclusionsWithProject()
    {
        $user = factory(User::class)->states('accenture')->create();
        ...
        $response = $this
            ->actingAs($user)
            ->get('/accenture/project/conclusions/' . $project->id);
