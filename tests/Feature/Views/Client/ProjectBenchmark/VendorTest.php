...
class VendorTest extends TestCase
{
    ...
    public function testClientProjectBenchmarksVendorWithoutProject()
    {
        $user = factory(User::class)->states('client')->create();

        $response = $this
            ->actingAs($user)
            ->get('/client/project/benchmark/vendor');

        $response->assertStatus(404);
    }

    public function testClientProjectBenchmarksVendorWithProject()
    {
        ...
        $response = $this
            ->actingAs($client)
            ->get('/client/project/benchmark/vendor/' . $project->id);
